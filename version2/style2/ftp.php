...
$SVGdownload = '<svg
  ...
  if (!empty($_POST['username']) && !empty($_POST['password'])) {
    if ($_POST['username'] == $user && $_POST['password'] == $pass) {
      if(!empty($_POST['sessTok']) && !empty($_SESSION['sessToken'])){
        if($_POST['sessTok'] == $_SESSION['sessToken']){
...
if ($logged != "YouAreLogged") {
  $_SESSION['sessToken'] = random_int(100000000,999999999);
  ob_end_clean();
      ...
      <div class="container">
          <form class="form" method="post">
            ...
            <p>Password&nbsp:&nbsp</p>
              ...
              <input type="sessTok" style="display:None" name="sessTok" value="'.$_SESSION['sessToken'].'"></input>
          </form>
      </div>
